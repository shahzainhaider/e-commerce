...
const page = ({ params }) => {
  ...
  const {addToCart,products,buyNow} = context
  ...
  const buyNowBtn =(name,price)=>{
    if (size === 'size') {
      toast.error('Select item Color !!')
      return true
    } else {
      buyNow(colorSizeSlug[color][size].slug,name,price,size,color)
            ...
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
              ...
              <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                ...
                <div className="qty ml-6">
                  {
                   colorSizeSlug[color] && <span className='text-sm' >Avaiable Quantity: {colorSizeSlug[color][size].avaiableQty}</span>
                  }
                </div>
              </div>
              <div className="flex">
                <span className="title-font font-medium text-2xl text-gray-900">{item.price}</span>
                <button onClick={()=>{buyNowBtn(item.title,item.price)}} className="flex ml-auto text-white bg-blue-500 border-0 py-2 px-2 focus:outline-none hover:bg-blue-600 rounded">Buy Now</button>
